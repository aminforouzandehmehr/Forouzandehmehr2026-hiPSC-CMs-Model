...
s.ICaL = 0.3;           % +30% ICaL conductance    
s.RyR  = 0.3;           % +30% release gain (RyR)  
s.IKs  = 0.5;           % +50% IKs conductance    
s.IKr  = 0.3;           % +30% IKr conductance
s.SERCAVmax = 0.3;      % +30% SERCA Vmax      
s.SERCaKm   = 0.3;      % -30% SERCA Km (higher affinity) --> you can decrease kd,cai in the code
s.RyRPo = 0.3;          % -30% RyR Po half activation constant (higher affinity)
s.INaK = 0.1;           % +10% Na/K pump (via phospholemman/PKA)
s.If = 0.1;             % +10% If
...
% Myofilament (TnI/MyBP-C): ↓Ca sensitivity, faster cycling/lusitropy
...
%% Run
Yn = Yc (end,:); Yn(33) = [0.1772];  %Yn(1) = -0.0752; % for clamp
...
for i= 1:size(Yc,1)
[~, dati]    = hiMCES2026(t(i), Yc(i,:), contrflag, GAC, stimFlag, classicVSoptic, tDrugApplication, INaFRedMed, INaLRedMed, INaKRedMed, ICaLRedMed, IKrRedMed, IKsRedMed, INaCaRedMed, IfRedMed, SERCARedMed, SERCAKmRedMed, RyRRedMed, RyRPoRedMed, ItoRedMed, ischemiaFlag, myFlags, IKatpSelectionString, fnp, z, flv, fmyo, gma, BlebFlag);
    INa(i)   = dati(1);
    If(i)    = dati(2);
    ICaL(i)   = dati(3);
    Ito(i)   = dati(4);
