...
% simpleCaTBiomarkers
% Compute Ca-transient biomarkers from the LAST complete beat (trough->trough):
...
%
% Notes:
%   • Rise thresholds use PRE-trough baseline (start trough).
%   • Decay thresholds use POST-trough baseline (end trough).
%   • If a fall threshold is not reached, we clamp to the segment end to avoid NaNs.
%   • If a rise threshold was already exceeded at segment start, we set rise time to segment start.
...
% ---- Rise crossings (with "already above" fix) ----
