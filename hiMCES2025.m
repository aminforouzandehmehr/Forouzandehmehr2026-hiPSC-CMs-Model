...
ISO_on        = time >= tDrugApplication && BlebFlag == 0;   % step; replace with a smooth ramp if desired
...
Vhalf_base_mV = -69;                 % from your formula (+69 inside exp)
k_mV          = 8;                   % slope factor
V_mV          = Y(1)*1000;
...
viscosity = 0.003;  % (normalised force s um-1)
  ...
  % *****************************************************    
    ...
    F_passive = passiveForcesISO(SL,time,tDrugApplication,fmyo);
    F_after = Afterload;
...
%eps = 5e-2;                % (s^-1) diffusion rate based on Fick's law eps = 2D/dx^2 --> dx = 10 um, D = 2.5e-10 m^2/s (ischemia-reperfusion) Def: 5.
...
eps     = D_m2s / (H_eff_m^2);  % ≈  was 5.63e-05 s^-1
eps = 2.56e-04;                 % 8.3333e-05;  alt for 5h: eps = 1.66e-04; 


ATP_basal = 0.01;          % Non-ATP-linked mitochondrial respiration (proton leak/maintenance). Zero or ATP_basal = 0.01;
J_SERCA = i_up;
...
ATP_leak  = ATP_basal;                   % e.g., small non-ATP-linked mitochondrial respiration (proton leak/maintenance) currently zero.
...
OCR_leak_raw  = ATP_basal;               % mM/s O2  (if ATP-linked, use phiATP.*ATP_basal)

OCR_nonXB_raw = OCR_SERCA_raw + OCR_NaK_raw + OCR_PMCA_raw + OCR_leak_raw;

alpha = 0.008;                         % Def: alpha = 0.01, gma = 22, rho = 0.25. OR: alpha = 0.008, gma = 22, rho = 0.25.
...
% 4) Finish state equation (your sign convention)
...
delta_m   = 1.0e-4;                               % m, represents a thin pericellular film (e.g., 50–200 µm), choose 50e-6 .. 2e-4 for 50–200 µm
Vc_L      = 8800 * 1e-15;                         % cell volume [L] (8800 µm^3)
V_peri_L  = 200e-6;                               % entire well volume [L] (use 200 µL from Emilia's table)
...
% OLD (incorrect if OCR is per-cell): 
% dY(33,1)  = OCR + eps*(O2s - Y(33));

% NEW (minimal change, pericellular Y(33)):
dY(33,1)  = OCR * scale_peri + eps*(O2s - Y(33));
%dY(33,1)  = OCR + eps*(O2s - Y(33));      % uses YOUR diffusion 'eps' (=5 s^-1)

%O2_local = max(Y(33), 0);
O2O = max(Y(33), 0);               % Def: = Y(33);
do2dt = dY(33,1);
PRF =  eps*(O2s-Y(33));

% Calculating pmol/min readouts to enable validation with in vitro data
% OCR is negative for consumption; I use minus sign (or abs) for readability
...
% Cells per well from seeding density (OLD)
%A_well_cm2 = 0.32;                         % cm^2, from Emilia's paper: many flat-bottom 96-wells are ~0.32–0.36 cm²
%rho        = 1.56e5;                       % cells/cm^2  from Emilia's paper
%N          = rho * A_well_cm2;             % cells/well
...
%% IPiezo1 (adapted from Forouzandehmehr et al., 2023: https://doi.org/10.22489/CinC.2023.359)

E_P = 60e-3;            % Volt 55e-3 from Paulina Czarnecki OR 0 mV from https://doi.org/10.1073/pnas.2215747120 and https://doi.org/10.1038/s41467-023-36931-0 OR 45e3 from Moroni et al., 2018 (https://doi.org/10.1038/s41467-018-03502-7)
P_Na = 1;
P_K  = 1;
P_Ca = 5;
Na_out = Nao; K_out = Ko; Ca_out = Cao;
Na_in = Y(18); K_in = Ki; Ca_in = Y(3);

% Calculates E_P using GHK equation (in volts)      E_Ca = 0.5*R*T/F*log(Cao/Y(3));
E_P = (R*T/F) * log( ...
        (P_Na*Na_out + P_K*K_out + P_Ca*4*Ca_out) / ...
        (P_Na*Na_in  + P_K*K_in  + P_Ca*4*Ca_in) );

E_P = 60e-3;    % Keep
%E_P = 0.5*R*T/F*log(Cao/Cai)

g_p = 0.3039;           % S/F from: https://doi.org/10.1038/s41467-023-36931-0
tau_p = 5e-3;            % Time constant     working: tau_p = 90e-3;
...
d = frc;    % frc is the tension generated by the myofilament model
mpi = 1.0/(1+exp(-(d-1.8)/0.95));

% Gate parameters
m0   = 0.1;
T50n = 2.5;
kTn  = 0.25;
mpi = m0 + (1 - m0) ./ (1 + exp(-(d - T50n)/kTn));
dY(34,1) = (mpi-Y(34))/tau_p;

% % Voltage-gated inactivation 
% Xs_inf = 1.0/(1.0+exp((-Y(1)*1000.0-0.0)/16.0)); 
% al_Xs    = 1/sqrt(1.0+exp((-60.0+Y(1)*1000.0)/50.0)); 
% be_Xs     = 900.0/(1.0+exp((+171.0-Y(1)*1000.0)/72.0)); 
% ta_Xs      = 1e-3*coeff_Q10_generico_tau * 0.3 * al_Xs * be_Xs;
% dY(35, 1)   = (Xs_inf-Y(35))/ta_Xs;
% 
% i_Pie = g_p*Y(34)*Y(35)*(Y(1)-E_P);                  % IPiezo1 (A/F)


% Voltage-gated inactivation (original form kept, but gated by sT)
Xs_inf = 1.0/(1.0+exp((-Y(1)*1000.0-50.0)/166.0));                 % Original: Xs_inf_raw = 1.0/(1.0+exp((-Y(1)*1000.0-50.0)/66.0));
al_Xs      = 1/sqrt(1.0+exp((-200.0+Y(1)*1000.0)/10.0)); 
be_Xs      = 900.0/(1.0+exp((+130.0-Y(1)*1000.0)/62.0)); 
ta_Xs  = 1e-3 * coeff_Q10_generico_tau * 0.3 * al_Xs * be_Xs;


% h_inf       = 1 / (1 + exp((Y(1)*1000 + 66.5)/2.8));
% tau_h       = GAC(18) * coeff_Q10_INa_tau * (0.00007 + 0.034 / (1 + exp((Y(1)*1000 + 41)/5.5) + exp(-(Y(1)*1000 + 41)/14)) + 0.0002 / (1 + exp(-(Y(1)*1000 + 79)/14)));
% dY(35, 1)   = (h_inf-Y(35))/tau_h;
% 
% j_inf       = h_inf;
% tau_j       = GAC(18) * coeff_Q10_INa_tau * 10*(0.0007 + 0.15 / (1 + exp((Y(1)*1000 + 41)/5.5) + exp(-(Y(1)*1000 + 41)/14)) + 0.002 / (1 + exp(-(Y(1)*1000 + 79)/14)));
% dY(36, 1)   = (j_inf-Y(36))/tau_h;

dY(35, 1) = (Xs_inf - Y(35)) / ta_Xs;


vv     = Y(1);
Vscale = 1e-3;                                 % 15–30 mV
DVeff  = Vscale * tanh( (vv - E_P) / Vscale );  % replaces (Y(1) - E_P)
DVeff  = vv - E_P;
i_Pie  = g_p * Y(34) * Y(35)  * DVeff;


%Jpizeo = 50* abs(i_Pie); % * cf;                    % Ca2+ influx of Piezo1 (mM/s)
Jpizeo = 1 * i_Pie;
Jpizeo2 = Jpizeo * Cm / (2.0 * Vc * F * 1.0e-18);    % Ca2+ influx of Piezo1 (mM/s)
mm = ta_Xs;                                          % Inactivation (s)
...
dY(1, 1) = -(i_K1+i_to+IKatp+i_Kr+i_Ks+i_CaL+i_NaK+i_Na+i_NaL+i_NaCa+i_PCa+i_f+i_b_Na+i_b_Ca+i_Pie-i_stim+I_bias);
...
dati = [INa, If, ICaL, Ito, IKs, IKr, IK1, INaCa, INaK, IpCa, IbNa, IbCa, Irel, Iup, Ileak, Istim, E_K, E_Na, INaL, F_passive, frc, ATPase, Lsarc, cvelo, JCaB, psph, IKatp, ronak, O2s, O2O, vcy, do2dt, finhib, Ko, ro1, ro2, fkatp, kon, i_Pie, Jpizeo2, mm, E_P, mpi, OCR, OCR_XB, OCR_nonXB, PRF, pmol_min_cell, pmol_min_well, OCR_NaK, OCR_leak, OCR_SERCA, OCR_PMCA];
%===============================================================================
% End
%===============================================================================
